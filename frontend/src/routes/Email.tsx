...
export default function Email() {
	...
	function handleForward() {
		...
		navigate(
			`/compose?subject=${encodeURIComponent(subject)}&fwdBody=${encodeURIComponent(fwdBody)}`,
		);
	}
		...
		<>
			<header class="content-header">
				...
				<div class="action-toolbar">
					...
					<Show when={showStar()}>
						<button
							class="icon-btn"
							classList={{ active: email()?.labelIds.includes("STARRED") }}
							onClick={handleToggleStar}
							aria-label={email()?.labelIds.includes("STARRED") ? "별표 해제" : "별표 표시"}
							disabled={actionLoading()}
						>
							<svg
								width="18"
								height="18"
								viewBox="0 0 24 24"
								fill={email()?.labelIds.includes("STARRED") ? "currentColor" : "none"}
								stroke="currentColor"
								stroke-width="2"
								stroke-linecap="round"
							>
					...
					<Show when={showReadToggle()}>
						<button
							...
							aria-label={email()?.labelIds.includes("UNREAD") ? "읽음 표시" : "읽지 않음 표시"}
							disabled={actionLoading()}
						>
							<svg
								width="18"
			...
			<div class="detail-scroll">
				<Show
					when={!loading()}
					fallback={
						<div class="loading-screen" style="height:auto;padding:60px">
							<div class="spinner" />
						</div>
					}
				>
						...
						{(e) => (
							...
							>
								...
								<div class="detail-body">
									<Show when={e().body.html} fallback={<pre>{e().body.text}</pre>}>
										<iframe
